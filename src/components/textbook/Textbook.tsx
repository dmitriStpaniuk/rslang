...
export const Textbook = () => {
    ...
    if (difficultWords) {
      setCards((cards) => {
        const cardsWithDifficult = cards.map(card => {
          return difficultWords.find(i => i.wordId === card.id) ? { ...card, isDifficult: true } : card
        })
  ...
  const handleDeleteDifficult = async (cardId: string) => {
    if (lewelDiff && user) {
      const newDifficultWord = await deleteWord(user.id, cardId)
      // setDifficultWords((difficultWords) => difficultWords?.filter(card => card.id === newDifficultWord))
    }
  }
      ...
      >
        ...
        <Grid container gap={1} justifyContent="center">
          {cards.map((card) => {
            ...
            return <CardTextbook key={card.word} card={card} lewelDiff={lewelDiff} isDifficult={card.isDifficult} handleAddDifficult={handleAddDifficult} />
          }
          )}
        </Grid>
        <Grid
